...
NSArray* LLK_Icons = nil;

@implementation LLKDBManager
+ (instancetype) shared {
    static id _sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedInstance = [[self alloc] init];
        LLK_Icons = [NSArray arrayWithObjects:
...
-(void) setup{
    ...
    [MagicalRecord setupCoreDataStackWithStoreNamed:LLKRecipesStoreName];
...
// 模拟数据
- (void) testData{
#define LLK_USER_NUMBER_MIN 20
#define LLK_USER_NUMBER_MAX 20

#define LLK_MESSAGE_NUMBER_MIN 1
#define LLK_MESSAGE_NUMBER_MAX 100
#define LLK_MESSAGE_DATE_INTERVAL 60*60

    
    [LLKUser MR_deleteAllMatchingPredicate:nil];
    [LLKMessage MR_deleteAllMatchingPredicate:nil];
    [LLKRecord MR_deleteAllMatchingPredicate:nil];
    
    NSUInteger userNumber = [self randomFrom:LLK_USER_NUMBER_MIN to:LLK_USER_NUMBER_MAX];
    for (int i=0;i<userNumber; i++) {
        LLKUser* user = [LLKUser MR_createEntity];
        NSUInteger index = [self randomFrom:0 to:LLK_Icons.count-1];
        user.iconURL = LLK_Icons[index];
        user.name = [NSString stringWithFormat:@"第%d个二货",i];
        user.userId = [NSString stringWithFormat:@"%d",i];
        
        LLKRecord* record = nil;
        
        NSUInteger messageNumber = [self randomFrom:LLK_MESSAGE_NUMBER_MIN to:LLK_MESSAGE_NUMBER_MAX];
        
        for (int i=0;i<messageNumber; i++) {
            LLKMessage* message = [LLKMessage MR_createEntity];
            message.date = @([[NSDate date] timeIntervalSince1970] - [self randomFrom:0 to:LLK_MESSAGE_DATE_INTERVAL]);
            message.sync = @(YES);
            message.type = @(0);
            message.value = [NSString stringWithFormat:@"第%d句话",i];
            message.fromMe = arc4random()%2 == 0 ? @(YES):@(NO);
            message.user = user;
            [user addMessagesObject:message];
            
            // 记录
            if (record == nil) {
                record = [LLKRecord MR_createEntity];
                record.user = user;
                record.lastMessage = message;
            }else{
                if (record.lastMessage.date.doubleValue < message.date.doubleValue) {
                    record.lastMessage = message;
                }
            }
        }
...
-(LLKMessage*) creatMessageWithType:(LLKMessageType)type value:(NSString*)value fromMe:(BOOL)fromMe user:(LLKUser*)user{
    LLKMessage* message = [LLKMessage MR_createEntity];
    message.date = @([[NSDate date] timeIntervalSince1970]);
    message.sync = @(NO);
    message.type = @(type);
    message.value = value;
...
-(NSArray*) findMessagesArrayWithUser:(LLKUser*)user offset:(NSUInteger)offset limit:(NSUInteger)limit{
    NSFetchRequest *request = [LLKMessage MR_requestAllSortedBy:@"date" ascending:YES withPredicate:[NSPredicate predicateWithFormat:@"%K = %@", @"user", user]];
    [request setFetchOffset:offset];
    [request setFetchLimit:limit];
    return [LLKMessage MR_executeFetchRequest:request];
}

#pragma mark - user

-(LLKUser*) findUserWithID:(NSString*)userID{
    return [LLKUser MR_findFirstByAttribute:@"userId" withValue:userID];
}
//-(LLKUser*) createUserWithID:(NSString*)userID{
//    LLKUser* user = [LLKUser MR_createEntity];
//    user.userId = userID;
//    return user;
//}
...
-(LLKUser*) replaceUserWithID:(NSString*)userID iconURL:(NSString*)iconURL name:(NSString*)name {
    LLKUser* user = [self findUserWithID:userID];
    if (!user) {
        user = [LLKUser MR_createEntity];
        user.userId = userID;
    }
...
- (NSArray*) findAllRecord{
    return [LLKRecord MR_findAllSortedBy:@"lastMessage.date" ascending:NO];
}
...
- (void) copyDefaultStoreIfNecessary;
{
    ...
    NSURL *storeURL = [NSPersistentStore MR_urlForStoreName:LLKRecipesStoreName];
    
	// If the expected store doesn't exist, copy the default store.
	if (![fileManager fileExistsAtPath:[storeURL path]])
    {
		NSString *defaultStorePath = [[NSBundle mainBundle] pathForResource:[LLKRecipesStoreName stringByDeletingPathExtension] ofType:[LLKRecipesStoreName pathExtension]];
